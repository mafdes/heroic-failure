...
export class StrengthChallenge {
  constructor() { this.attributeId = "strength"; this.reset(); }
  reset() { this.round = 0; this.charge = 0; this.score = 1; this.status = "intro"; this.hasCharged = false; this.zoneOffset = 0; this.zoneDirection = 1; }
  update(delta, pressed, held, released) {
    if (this.status === "intro" && pressed) { this.status = "countdown"; this.countdown = 3; return; }
    if (this.status === "countdown") { this.countdown -= delta; if (this.countdown <= 0) this.status = "ready"; return; }
    if (this.status === "ready" && pressed) { this.status = "playing"; this.charge = 0; this.hasCharged = false; return; }
    if (this.status !== "playing") return;
    const config = roundConfig(this.round);
    if (this.round >= 4) {
      this.zoneOffset += this.zoneDirection * .055 * delta;
      if (Math.abs(this.zoneOffset) >= .13) this.zoneDirection *= -1;
    }
  ...
  resolveRound(config) {
    const zoneStart = this.zoneStart(config);
  ...
  zoneStart(config) { return .5 + (this.round >= 4 ? this.zoneOffset : 0) - config.zone / 2; }
  draw(ctx) {
    ...
    const config = roundConfig(this.round); const x = 130; const y = 265; const width = 700; const height = 42; const zoneStart = x + width * this.zoneStart(config);
    ...
    ctx.fillStyle = "#4a3d51"; ctx.fillRect(x, y, width, height); ctx.fillStyle = "#a9683e"; ctx.fillRect(zoneStart, y, width * config.zone, height); ctx.fillStyle = "#f3c46b"; ctx.fillRect(x, y, width * this.charge, height); this.copy(ctx, this.round >= 4 ? "LA ZONA SE MUEVE. EL CONTRATO LO PERMITE." : "MANTÉN ESPACIO / CLIC Y SUELTA EN LA ZONA", 390);
  }
